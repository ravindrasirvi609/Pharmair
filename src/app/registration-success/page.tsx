    ...
    const fetchRegistration = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setError(response.data.message || 'Failed to load registration details');
        }
      } catch (error: any) {
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-green-600 px-6 py-4 text-center">
          ...
          <p className="text-green-100 mt-2">You are now officially registered for the Pharmair Conference.</p>
        </div>
        
        <div className="p-6">
          <div className="mb-8">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Your Registration Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-gray-50 p-4 rounded-md">
                ...
                <p className="font-medium">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${registration?.registrationStatus === 'Confirmed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
              ...
              <div className="bg-gray-50 p-4 rounded-md">
                ...
                <p className="font-medium">
                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${registration?.paymentStatus === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
          ...
          {registration?.qrCodeUrl && (
            <div className="mb-8 text-center">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Your Conference QR Code</h3>
              <div className="inline-block bg-white p-4 rounded-lg border">
                <Image 
                  ...
                  className="mx-auto" 
                />
                <p className="mt-2 text-sm text-gray-500">Please save this QR code. It will be used for check-in at the conference.</p>
              </div>
            </div>
          )}
          ...
          {registration?.paymentStatus !== 'Completed' && (
            <div className="mb-8 bg-yellow-50 border border-yellow-200 rounded-md p-4">
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-yellow-800">Attention: Payment Pending</h3>
                  <div className="mt-2 text-sm text-yellow-700">
                    <p>Your registration is pending payment. Please complete the payment to confirm your attendance.</p>
                  </div>
                  <div className="mt-4">
                    <div className="-mx-2 -my-1.5 flex">
                      <Link href={`/payment?registrationId=${registration?._id}`} className="bg-yellow-800 px-4 py-2 rounded-md text-sm font-medium text-white hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500">
                        Complete Payment
                      </Link>
                    </div>
                  </div>
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-2">
              <li className="flex items-start">
                ...
                <span>Check your email for the registration confirmation details.</span>
              </li>
              <li className="flex items-start">
                <svg className="h-5 w-5 text-green-500 mr-2 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
